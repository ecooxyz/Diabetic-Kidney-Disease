...
library(tidyverse)
library(ggpubr)
rt<-read.csv("merge矩阵new.csv",header = T,sep = ",",row.names = 1)
...
fegene<-read.table("机器学习交集.txt",header = T,sep = "\t")
fegene<-fegene[,1]
...
ggsave(p, filename =paste0("merge","差异表达图.pdf"), width = 15, height = 6)

drawdata=drawdara
drawdata$condition<-factor(drawdata$condition,levels = c('DN','Control'))
my_comparisons <- list( c("DN", "Control") )
...
for (gene in Index) {
  ...
  ggsave(p,filename = paste(gene,'差异表达.pdf',sep=' '),width = 5.6,height = 4.22)
  print(gene)
}
