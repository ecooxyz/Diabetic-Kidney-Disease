...
rt=outTab
genename=read.table("ALL差异基因.txt",header = T)
rt=rt[genename$x,]
dim(rt)

###载入性状数据
traitData = read.table("WGCNA样本.txt",row.names=1,header=T,comment.char = "",check.names=F,sep="\t")
...
pdf(file="8_Module_trait.pdf",width=10,height=6)
...
for (i in 1:length(geneInfo$moduleColor)) {
  bbb<-geneInfo %>%
    dplyr::filter(moduleColor==geneInfo$moduleColor[i]) %>%
    dplyr::select(probes)
  write.table(bbb,file = paste0(geneInfo$moduleColor[i],"基因.txt"),row.names = F)
}
